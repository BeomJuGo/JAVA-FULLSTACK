            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    ...
                    <% } else { %>
                        ...
                        <% if (isAdmin) { %>
                            <button class="px-4 py-2 border-2 border-black bg-yellow-300 hover:bg-yellow-400" onclick="location.href='<%=ctx%>/admin.jsp'">관리자 페이지</button>
                        <% } %>
                    <% } %>
                </div>
            </div>
...
<script>
  ...
  async function fetchAndRender() {
    ...
    try {
      // (디버깅용) 네트워크 확인에 도움
      // console.log('[FETCH]', LIST_API + '?' + params.toString());
      ...
      grid.innerHTML =
        '<div class="col-span-2 text-center text-red-600 py-8 border-2 border-red-300 rounded-xl">' +
        '목록을 불러오지 못했습니다.' +
        '</div>';
  ...
  function render() {
    ...
    if (!pageItems.length) {
      grid.innerHTML =
        '<div class="col-span-2 text-center text-gray-500 py-8 border-2 border-dashed border-gray-300 rounded-xl">' +
        '등록된 가게가 없습니다.' +
        '</div>';
    } else {
        ...
        return '' +
          '<div class="border-2 border-black p-4 rounded-xl">' +
            ...
            '<button class="px-3 py-2 border-2 border-black hover:bg-gray-100" ' +
              'onclick="location.href=\'' + ctx + '/review.jsp?name=' + encodeURIComponent(item.name) + '\'">' +
              '리뷰 보기' +
            '</button>' +
          '</div>';
      }).join('');
  ...
  function getTotalPages() {
    return Math.max(1, Math.ceil(cachedItems.length / pageSize));
  }
